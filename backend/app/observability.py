...
class RuntimeObservability:
    ...
    def __init__(self) -> None:
        ...
        self._recent_requests: deque[dict] = deque(maxlen=240)
    ...
    def service_snapshot(self) -> dict:
        ...
        return {
            ...
            "environment": {
                "FF_PORT": os.environ.get("FF_PORT", "10010"),
...
def build_findings(snapshot: dict) -> list[dict]:
    ...
    p95 = float(((requests.get("lifetime") or {}).get("latency_ms") or {}).get("p95") or 0)
    if p95 >= 1000.0:
        add(
            "warning",
            "slow_http_p95",
            "API P95 延迟偏高",
            f"生命周期 P95 为 {p95:.1f} ms",
    ...
    for panel in ((snapshot.get("data") or {}).get("panels") or []):
        if panel.get("state") == "error" and panel.get("required", True):
            add(
                "critical",
                "panel_load_error",
                f"{panel.get('market', 'unknown')} 面板加载失败",
                panel.get("load_error") or panel.get("source_error") or "unknown",
